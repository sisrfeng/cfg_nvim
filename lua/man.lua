-- 我之前DIY过syntax或autoload或ftplugin下的man.vim, 后来改名为mm.vim(但没处理man命令了?)
    ...
    for char in line:gmatch('[^\128-\191][\128-\191]*') do
        if overstrike then
            ...
            elseif prev_char == '_' then
                -- Even though underline is strictly what this should be. 
                -- <bs>_ was used by nroff to
                -- indicate italics which wasn't possible on old typewriters so underline was used.
                -- Modern terminals now support italics so lets use that now.
                -- See:
...
-- replace spaces in a man page name with underscores  intended for PostgreSQL,
-- which has man pages like 'CREATE_TABLE(7)';
-- while editing SQL source code,
-- it's nice to visually select 'CREATE TABLE'  and hit 'K',
...
local function get_page(path, silent)
    ...
    -- print('manwidth: ', manwidth)  -- 999
    local cmd = localfile_arg and { 'man', '-l', path } or { 'man', path }

    -- Force MANPAGER=cat to ensure
    -- Vim is not recursively invoked (by man-db).
    -- http://comments.gmane.org/gmane.editors.vim.devel/29085
    -- Set MAN_KEEP_FORMATTING 
    -- so Debian man doesn't discard backspaces.
...
local function put_page(page)
    ...
    -- XXX: nroff justifies text by 
    -- filling it with whitespace.
