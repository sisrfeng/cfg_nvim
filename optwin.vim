" These commands create the option window (:help options)
"
...
if buf >= 0
    ...
    if len(winids) > 0
        if win_gotoid(winids[0]) == 1  | finish  | endif
    endif
endif

let s:cpo_save = &cpo  | set cpo&vim
    " Make sure the '<' flag is not included in 'cpoptions',
    " otherwise <CR> would  not be recognized.

" function to be called when <CR> is hit in the option-window
fun! <SID>CR()
    " If on a continued comment line,
    " go back to the first comment line
