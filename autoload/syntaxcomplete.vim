...
" Set completion with CTRL-X CTRL-O to autoloaded function.
    ...
    if exists('+omnifunc')
      "\ exists
            "\ &option-name    Vim option (only checks if it exists,  not if it really works)
            "\ +option-name    Vim option that works.

        " Do not set the option if already set since
        " this  results in an E117 warning.
...
" but if you define a buffer local  variable you can override this on a per filetype.
if !exists('g:omni_syntax_ignorecase')  | let g:omni_syntax_ignorecase = &ignorecase  | endif

" Indicates whether we should use the iskeyword option to determine
" how to split words.
" But if you define a buffer local variable you can override this on a per filetype.
if !exists('g:omni_syntax_use_iskeyword')  | let g:omni_syntax_use_iskeyword = 1  | endif

" When using iskeyword,
" this setting controls whether the characters
" should be limited to single byte characters.
if !exists('g:omni_syntax_use_single_byte')  | let g:omni_syntax_use_single_byte = 1  | endif

" When using iskeyword,
" this setting controls whether the characters
" should be limited to single byte characters.
if !exists('g:omni_syntax_use_iskeyword_numeric')  | let g:omni_syntax_use_iskeyword_numeric = 1  | endif

" Only display items in the completion window that are at least
" this many characters in length.
if !exists('g:omni_syntax_minimum_length')  | let g:omni_syntax_minimum_length = 0  | endif

" This script will build a completion list based on the syntax  elements defined by
" the files in $VIMRUNTIME/syntax
